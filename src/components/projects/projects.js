// components/home/projects.js  (or wherever you place components)
   ...
   return `
      <article class="proj-card group
         ...
         <div class="p-5 md:p-6 flex flex-col gap-4">
            ...
            <!-- Description -->
            <p class="text-sm/6 text-gray-500 dark:text-gray-400 line-clamp-3">${description}</p>
...
// ── Slider logic ──────────────────────────────────────────────────────────────
const goToSlide = (projectId, nextIndex) => {
...
// ── Auto-advance per card ─────────────────────────────────────────────────────
const startAutoplay = (projectId, total) => {
   if (total <= 1) return;
   setInterval(() => {
      const current = sliderState[projectId] ?? 0;
